...
class HistoryManager:
    ...
    def __init__(
        ...
        self._online = online
        self._market = (market or "poloniex").lower()
        if self._online:
    ...
    def initialize_db(self):
        ...
        db_path = get_database_path(self._database_file)
    ...
    def get_global_panel(self, start, end, period=300, features=('close',)):
        ...
        # Build a 3D numpy array [feature, coin, time] instead of deprecated pandas.Panel
        num_features = len(features)
        num_coins = len(coins)
        num_times = len(time_index)
        values = np.full((num_features, num_coins, num_times), np.nan, dtype=np.float32)
        # Map timestamp to index for quick assignment
            ...
            for coin_idx, coin in enumerate(coins):
                for feat_idx, feature in enumerate(features):
                    ...
                    if not serial_data.empty:
                        for ts, val in serial_data.itertuples():
                            pos = time_pos.get(ts)
                            if pos is not None:
                                try:
                                    values[feat_idx, coin_idx, pos] = np.float32(val)
                                except Exception:
                                    # In case of NaN or type issues, leave as NaN
                                    pass
            # Fill NaNs along the time axis for each [feature, coin]
            for fi in range(num_features):
                for ci in range(num_coins):
                    series = pd.Series(values[fi, ci, :], index=time_index)
                    series = series.fillna(method="ffill").fillna(method="bfill")
                    values[fi, ci, :] = series.values.astype(np.float32)
        finally:
            connection.commit()
            connection.close()
        # Lightweight panel-like object with only the used attributes
        class _PanelLike(object):
            def __init__(self, values, major_axis, minor_axis):
                self.values = values
                self.major_axis = pd.Index(major_axis)
                self.minor_axis = pd.DatetimeIndex(minor_axis)
        return _PanelLike(values=values, major_axis=coins, minor_axis=time_index)
