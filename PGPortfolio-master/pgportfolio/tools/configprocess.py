...
def fill_default(config):
    ...
    fill_trading_default(config["trading"])
...
def fill_trading_default(trading_config):
    set_missing(trading_config, "provider", "")
    set_missing(trading_config, "api_key", "")
    set_missing(trading_config, "api_secret", "")
    set_missing(trading_config, "passphrase", "")
    set_missing(trading_config, "api_key_env", "OKX_API_KEY")
    set_missing(trading_config, "api_secret_env", "OKX_API_SECRET")
    set_missing(trading_config, "passphrase_env", "OKX_PASSPHRASE")
    set_missing(trading_config, "use_simulated", True)
    set_missing(trading_config, "paper_trading", True)
    set_missing(trading_config, "td_mode", "cash")
    set_missing(trading_config, "live_steps", 0)
    set_missing(trading_config, "min_order_btc", 5e-4)
    set_missing(trading_config, "symbol_overrides", {})
    set_missing(trading_config, "quote_currency", "BTC")
    set_missing(trading_config, "base_url", "")
    set_missing(trading_config, "timeout", 10)
