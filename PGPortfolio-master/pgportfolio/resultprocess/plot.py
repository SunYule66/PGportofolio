...
def plot_backtest(config, algos, labels=None):
    ...
    rc("font", **{"family": "sans-serif", "sans-serif": ["DejaVu Sans", "Arial", "SimHei"],
                  "size": 8})
    ...
    fig.autofmt_xdate()
    plt.savefig("result.png", bbox_inches='tight', pad_inches=0, dpi=150)
    plt.show()
...
def _load_from_summary(index, config):
    ...
    """
    dataframe = pd.read_csv("./train_package/train_summary.csv").set_index("net_dir")
